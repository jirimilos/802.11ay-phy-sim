...
disp(' *******************************************************************')
disp(' ');
disp([' IEEE ', wifi_standard, ' > ', wifi_params.MCS(i_mcs).coding_type, ', ', wifi_params.MCS(i_mcs).MCS, ', M = ', num2str(wifi_params.MCS(i_mcs).M), ', CR = ', sprintf('%2.2f',wifi_params.MCS(i_mcs).CR), ', ', num2str(wifi_params.MCS(i_mcs).N_ss), ' Spatial stream(s):']);
disp(' ');
